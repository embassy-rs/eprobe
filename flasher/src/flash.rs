...
#[derive(Parser)]
pub struct Cmd {
    /// Serial number of the probe to use (optional)
    #[arg(short, long)]
    serial: Option<String>,

    /// Bootloader ELF file path
    #[arg(short, long, default_value = "../build/bootloader.elf")]
    bootloader: String,

    /// Application ELF file path
    #[arg(short, long, default_value = "../build/application.elf")]
    application: String,
}
...
pub fn run(cmd: &Cmd) {
    flash_probe_rs(cmd.serial.as_deref(), &cmd.bootloader, &cmd.application);
}

fn flash_probe_rs(serial: Option<&str>, bootloader_path: &str, application_path: &str) {
    let lister = Lister::new();

    let probes = lister.list_all();
    let probe_info = if let Some(serial) = serial {
        probes
            .into_iter()
            .find(|p| p.serial_number.as_deref() == Some(serial))
            .expect("Probe with specified serial number not found")
    } else {
        probes.into_iter().next().expect("No probes found")
    };

    info!("Using probe: {:?}", probe_info);
    ...
    let mut bl = File::open(bootloader_path).expect("Failed to open bootloader file");
    let mut app = File::open(application_path).expect("Failed to open application file");

    info!("Loading bootloader: {}", bootloader_path);
    loader
        .load_image(&mut sess, &mut bl, Format::Elf(ElfOptions::default()), None)
        .unwrap();

    info!("Loading application: {}", application_path);
    loader
        .load_image(
            &mut sess,
            &mut app,
